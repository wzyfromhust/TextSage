...
struct ChatView: View {
    ...
    @State private var showDebugAlert: Bool = false // 复用调试弹窗状态
    ...
    var body: some View {
        ...
        .alert(isPresented: $showDebugAlert) {
            Alert(
                title: Text(debugAlertTitle),
                message: Text(debugAlertMessage),
    ...
    func sendMessage() {
        ...
        // 显示调试信息
        debugAlertTitle = "API请求"
        debugAlertMessage = "正在发送请求给AI服务...\n模型ID: \(AIService.shared.modelId)\n流式输出: \(AIService.shared.useStreamingOutput ? "开启" : "关闭")\n消息数: \(messagesForAPI.count)\nAPI基础URL: \(AIService.shared.baseURL)"
        showDebugAlert = true
        ...
        if AIService.shared.useStreamingOutput {
            ...
            AIService.shared.sendChatRequestStream(messages: messagesForAPI, onChunk: { chunk in
                ...
                case .failure(let error):
                    ...
                    // 显示错误信息
                    debugAlertTitle = "API错误"
                    debugAlertMessage = "获取AI回复失败: \(error.localizedDescription)"
                    showDebugAlert = true
                }
    ...
    // 保留模拟响应方法（仅用于测试）
    func simulateAIResponse(to query: String, withContext context: String) -> String {
        if context.isEmpty {
            return "我将尽力回答你的问题：\(query)\n\n这是一个模拟的回复，在真实应用中我会连接到AI API并提供真实的回答。"
        } else {
            return "基于你选择的文本：\"\(context.prefix(50))...\"\n\n针对你的问题：\(query)\n\n这是一个模拟的回复，在真实应用中我会连接到AI API并提供基于选中文本的真实回答。"
        }
    }
}
